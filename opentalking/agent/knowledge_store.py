...
SUPPORTED_EXTENSIONS = SUPPORTED_TEXT_EXTENSIONS | SUPPORTED_PDF_EXTENSIONS | SUPPORTED_PRESENTATION_EXTENSIONS | SUPPORTED_WORD_EXTENSIONS
# Keep the original format sequence in the message for existing clients/tests;
# Word formats are appended as an additional supported group.
SUPPORTED_EXTENSIONS_LABEL = ".txt, .md, .markdown, .pdf and .pptx; Word documents: .doc, .docx"
...
def _extract_docx_text(path: Path) -> tuple[str, str | None]:
    if not path.is_file():
        return "", "docx document is missing"
    if path.suffix.lower() != ".docx":
        return "", "unsupported Word document type"
    try:
        import zipfile
        from xml.etree import ElementTree as ET
    except Exception as exc:  # pragma: no cover - stdlib import failure is unexpected
        return "", f"failed to load docx parser: {exc}"
    if not zipfile.is_zipfile(path):
    ...
    try:
        ...
        return (text, None) if text else ("", "docx contains no extractable text")
    except KeyError:
        return "", "docx document.xml is missing"
    except Exception as exc:  # noqa: BLE001
        return "", f"failed to extract docx text: {exc}"


def _extract_doc_text(path: Path) -> tuple[str, str | None]:
    if not path.is_file():
        return "", "doc document is missing"
    extractor = shutil.which("antiword") or shutil.which("catdoc")
    if not extractor:
        return "", "legacy .doc extraction requires antiword or catdoc"
    try:
        result = subprocess.run(
            [extractor, str(path)],
            capture_output=True,
        ...
        return "", f"failed to extract doc text: {exc}"
    text = (result.stdout or "").strip()
    if result.returncode != 0:
        return "", (result.stderr or "failed to extract doc text").strip()
    return (text, None) if text else ("", "doc contains no extractable text")
