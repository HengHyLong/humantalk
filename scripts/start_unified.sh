...
start_admin() {
  ...
  local admin_api_mode="${VITE_ADMIN_API_MODE:-real}"
  ...
  echo "  mode:  $admin_api_mode"
  if ! (cd "$admin_dir" && export VITE_BACKEND_PORT="$admin_backend_port" VITE_ADMIN_API_MODE="$admin_api_mode" && npm run build >>"$log_file" 2>&1); then
  ...
  (
    cd "$admin_dir"
    export VITE_BACKEND_PORT="$admin_backend_port" VITE_ADMIN_API_MODE="$admin_api_mode"
