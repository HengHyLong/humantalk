...
`AUDIO2VIDEO_PLAYBACK_AUDIO_RESERVE_MS` 用于避免等待视频队列时耗尽旧音频。日志中的 `audio_buffer_ms=等待前->等待后->新音频入队后` 可以确认新音频没有在对应视频之前播放；等待后的数值通常会停在储备线附近。`av_queue_skew_ms` 是音频减视频的排队时长估算，可用于判断是哪条轨道持续积压。
