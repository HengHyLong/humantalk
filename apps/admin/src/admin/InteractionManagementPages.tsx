...
export function WelcomeConfigPage({ canWrite, initialExhibitionId }: Props) {
  ...
  const create = () => {
    ...
    const next: WelcomeConfig = {
      ...
      wakePrompt: "🎤想了解展会资讯？叫一声「小美小美」，小美随时为您服务！",
  ...
  const save = async () => {
    if (!current) return;
    const configToSave = { ...current, wakeWords: parseWakeWordsInput(wakeWordsInput) };
    const validationError = validateWakeWords(configToSave);
    if (validationError) { setError(validationError); return; }
    if (!availableWelcomeScripts.some((script) => script.id === configToSave.scriptId)) {
      setError("请选择当前展会已启用的迎宾话术模板。");
      return;
    }
    try {
      const next = await adminApi.saveWelcomeConfig(configToSave);
    ...
    {current ? <Card className="mt-4 p-5">
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-2">
        <div className="space-y-4">
          ...
          {current.triggers.includes("唤醒词") ? <div className="space-y-4"><div><label htmlFor="wake-words-input" className="block text-xs font-semibold text-slate-600">唤醒词（逗号或换行分隔）<textarea id="wake-words-input" data-testid="wake-words-input" rows={3} value={wakeWordsInput} onChange={(event) => { setWakeWordsInput(event.target.value); setError(""); }} placeholder={"你好小展\n小展小展"} className="mt-2 w-full resize-y rounded-xl border border-slate-200 px-3 py-2.5 text-sm font-normal text-slate-800 outline-none focus:border-cyan-400" /></label><p className="mt-2 text-[11px] text-slate-400">支持英文逗号、中文逗号、顿号或回车换行；输入时不会自动替换，保存时统一整理。最多 5 个，每个 2～12 个字符。数字人处于休眠状态时，只响应包含唤醒词的语音。</p></div><Field label="休眠/唤醒提示语" textarea value={current.wakePrompt} onChange={(value) => { setEditing({ ...current, wakePrompt: value }); setError(""); }} placeholder="提示观众如何唤醒数字人" /><label className="block text-xs font-semibold text-slate-600">无对话进入休眠时间（秒）<input type="number" min={10} max={600} step={1} value={current.wakeActiveSeconds} onChange={(event) => { setEditing({ ...current, wakeActiveSeconds: Number(event.target.value) }); setError(""); }} className="mt-2 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm font-normal text-slate-800 outline-none focus:border-cyan-400" /><p className="mt-2 text-[11px] font-normal text-slate-400">唤醒后，每次有效语音对话都会重新计时；连续无对话达到该时长后再次进入休眠。范围 10～600 秒。</p></label></div> : null}
