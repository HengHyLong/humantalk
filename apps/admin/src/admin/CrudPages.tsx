...
const IMAGE_ONLY_AVATAR_MODELS = new Set(["vidu"]);
...
function customAvatarImageError(file: File): string | null {
  const extension = fileExtension(file);
  const isImage = file.type.toLowerCase().startsWith("image/") || CUSTOM_AVATAR_IMAGE_EXTENSIONS.includes(extension);
  if (!isImage) return "Vidu 仅支持 JPEG、PNG 或 WebP 图片，不支持视频文件。";
...
export function EnhancedAvatarPage({ onDebug }: { onDebug?: (avatarId: string) => void }) {
  ...
  const reload = useCallback(() => {
    ...
    void Promise.allSettled([openTalkingClient.listAvatars(), openTalkingClient.listModels()]).then(([avatarResult, modelResult]) => {
      ...
      if (modelResult.status === "fulfilled") {
        ...
        setModels(Array.from(new Set([...modelResponse.models, "video", "vidu"])));
        setForm((current) => ({
          ...current,
  ...
  const add = async () => {
    ...
    } else if (IMAGE_ONLY_AVATAR_MODELS.has(form.model)) {
      if (!form.file || isCustomAvatarVideo(form.file)) {
        setError("Vidu 仅需上传一张图片，不支持视频文件。");
        return;
      }
      const imageError = customAvatarImageError(form.file);
      if (imageError) {
        setError(imageError);
        return;
      }
