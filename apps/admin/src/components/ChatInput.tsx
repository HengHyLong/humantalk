...
export function ChatInput({
  ...
  }, [disabled, enterVoiceMode, teardownVoicePipeline, voiceCaptureEnabled, voiceMode]);
    ...
    <div className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
      <div className="flex flex-col gap-2">
        {voiceMode ? (
          <button
            type="button"
            className="group w-full rounded-xl text-left transition focus:outline-none focus:ring-2 focus:ring-indigo-300 focus:ring-offset-2"
            title="点击关闭语音交互"
            aria-label="关闭语音交互"
            aria-pressed="true"
          ...
          >
            <div
              className="rounded-xl border border-indigo-200 bg-gradient-to-b from-white to-indigo-50/70 px-4 py-3 group-hover:border-indigo-300 group-hover:from-indigo-50/30"
              role="status"
              aria-live="polite"
            >
              <div className="flex items-center justify-center gap-2.5">
                <span className="h-2.5 w-2.5 animate-pulse rounded-full bg-indigo-500 shadow-[0_0_0_5px_rgba(99,102,241,0.12)]" />
                <p className="text-base font-semibold text-slate-800">我正在听，请说话</p>
              </div>
              <div className="mx-auto mt-3 flex h-9 max-w-[20rem] items-center justify-center gap-2 rounded-full bg-indigo-500 px-8 shadow-[0_8px_24px_rgba(99,102,241,0.28)]">
                {[14, 22, 30, 38, 28, 20, 13].map((height, index) => (
                  <span
                    key={index}
                    className={`w-1.5 rounded-full bg-white/90 transition-[height,opacity] duration-200 ${segmentHot ? "animate-pulse opacity-100" : "opacity-75"}`}
        ...
        {!voiceMode ? <div className="flex flex-col gap-2 md:flex-row md:items-end">
          <textarea
          ...
          {voiceCaptureEnabled ? (
            <button
              ...
              aria-label={voiceMode ? "关闭语音交互" : "开启语音交互"}
              disabled={(!voiceMode && voiceBusy) || ftAudioBusy}
              onClick={(e) => {
                e.preventDefault();
                if (disabled) {
