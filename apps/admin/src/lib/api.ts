...
export function buildWsUrl(path: string): string {
  ...
  try {
    ...
    const apiBase = API_BASE.trim().replace(/^\/+/, "").replace(/\/+$/, "");
...
function normalizedApiBase(): URL {
  ...
  // API_BASE historically defaulted to `api`. A relative URL is resolved
  // against the current document path, so `/asset/avatar` incorrectly
  // produced `/asset/api/...`. Keep absolute backend URLs supported, while
  // treating relative configured paths as root-relative API paths.
  if (/^[a-z][a-z\d+.-]*:\/\//i.test(base)) {
    return new URL(base);
  }

  const rootRelativeBase = base.startsWith("/") ? base : `/${base}`;
